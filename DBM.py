import pandas as pd
from headhunter_parsr import HeadHunter
import psycopg2
...
class DBManager:
    ...
    def insert_data(self):
        """Добавляет информацию в таблицы"""
        conn = psycopg2.connect(host=self.host, database=self.database, user=self.user, password=self.password)
        conn.autocommit = True
        cur = conn.cursor()
        hh = HeadHunter()
        ...
        for item in data_employers:
            ...
            cur.execute(f"""INSERT INTO employers (company_id, company_name)
                            VALUES ('{employers_id}', '{company_name}')
    ...
    def get_all_vacancies(self):
        """Получает список вакансий - компании, вакансии, средней зарплаты, и ссылки"""
        conn = psycopg2.connect(host=self.host, database=self.database, user=self.user, password=self.password)
        conn.autocommit = True
        cur = conn.cursor()
        cur.execute(f"""SELECT company_name, vacancy_name, salary_from, salary_to, url
                        FROM vacancies
                        INNER JOIN employers ON vacancies.company = employers.company_id""")
        result = cur.fetchall()
    ...
    def get_avg_salary(self):
        ...
        conn.autocommit = True
        cur = conn.cursor()
        cur.execute(f"""SELECT company_name, ROUND(AVG(salary_from)) as avg_salary
                        FROM vacancies
                        INNER JOIN employers ON vacancies.company = employers.company_id
                        GROUP BY company_name""")
        result = cur.fetchall()
